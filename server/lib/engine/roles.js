...
// 人数 -> 角色组成（定死的规则表）
export function compositionFor(n, mode) {
    const table = {
        simple: {
    ...
    if (!row)
        throw new Error(`人数 ${n} 不在 ${mode} 模式支持范围内`);
    return [...row];
}
export function modeForPlayerCount(n) {
    if (n <= 4)
        return "simple";
    if (n <= 8)
        return "standard";
...
export const ROLE_ORDER = ["werewolf", "seer", "witch", "hunter", "idiot", "villager"];
