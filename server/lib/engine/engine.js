...
import { compositionFor, modeForPlayerCount } from "./roles.js";
...
export class WerewolfGame {
    ...
    async decideFor(player, requiredAction, phaseLabel) {
        if (player.isHuman) {
    ...
    waitHumanInput(player, requiredAction, phaseLabel) {
    ...
    get pendingHumanSeat() {
        return this.pendingHuman?.playerId;
    }
    get pendingHumanAction() {
    ...
    autoDecision(player, requiredAction, _phaseLabel) {
    ...
    privateInfoFor(p) {
        ...
        if (p.role === "werewolf") {
            ...
            if (mates.length > 1)
                info.push(`你的狼同伴：${mates.map((m) => `${m.seat}号 ${m.name}`).join("、")}`);
        }
        if (p.role === "seer") {
    ...
    resolveNightDeaths() {
        ...
        const push = (id, cause) => {
            if (seen.has(id))
                return; // 刀与毒同目标时只记一次（毒优先）
            seen.add(id);
            died.push({ id, cause });
        };
        if (this.witchPoisonTarget !== undefined)
            push(this.witchPoisonTarget, "poison");
    ...
    async announceNight() {
        ...
        if (hunterDied) {
            if (hunterDied.cause === "poison") {
                ...
                this.pendingHunter = { id: hunterDied.id, cause: "wolf" };
            }
        }
        // 遗言：第一个死亡的玩家（若有多个，取座位最前）
        const first = [...died].sort((a, b) => this.byId(a.id).seat - this.byId(b.id).seat)[0];
        if (first)
            this.pendingLastWords = first.id;
    }
    async processLastWordsAndHunter() {
     ...
     */
    emitHumanTurn(player, requiredAction, phaseLabel) {
        this.emit("human_turn", { playerId: player.id, seat: player.seat, action: requiredAction, label: phaseLabel });
    }
    systemNote(message, secret = false) {
        this.emit("system", { message, secret });
    }
    /** 真人超时托管的默认合法决策（公开给驱动器，供无状态模式下写入日志） */
    autoDecisionFor(player, requiredAction, phaseLabel = "") {
...
export function resolveMode(n, mode) {
