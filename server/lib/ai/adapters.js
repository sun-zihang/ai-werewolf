...
export function thinkingInstruction(level) {
 ...
 */
export function buildThinkingParams(providerId, level) {
    switch (providerId) {
        case "openai":
...
export async function callProvider(req) {
    ...
    try {
        ...
        if (err?.name === "AbortError")
            throw new Error(`请求超时（${LEVEL_META[req.thinkingLevel].timeoutMs / 1000}s）`);
        throw err;
    }
    finally {
        clearTimeout(timer);
